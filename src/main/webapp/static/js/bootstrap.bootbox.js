 ...
 */
window.bootbox = window.bootbox || (function(document, $) {
    ...
    that.alert = function(/*str, label, cb*/) {
        ...
        var div = that.dialog(str, {
        	...
        	"header"  : '<i class="icon-ok-sign"></i> 消息',
            // callback or true to just close the dialog
            "onEscape": cb || true
        });
        div.off("keyup.alert").on("keyup.alert",function(e){
         	var key = e.which;
         	if(key == 13){
         		div.modal("hide");
         	}
    ...
    that.confirm = function(/*str, labelCancel, labelOk, cb*/) {
        ...
        var div = that.dialog(str, [
     	     ...
     	     },
     	     {
             	...
             	"header"  : '<i class="icon-info-sign"></i> 确认',
                 // escape key bindings
                 "onEscape": cancelCallback
             });
             
             div.off("keyup.confirm").on("keyup.confirm",function(e){
             	var key = e.which;
             	if(key == 13){
             		div.modal("hide");
             		confirmCallback();
    ...
    that.dialog = function(str, handlers, options) {
        ...
        while (i--) {
            ...
            //RICKY 删除href属性
               ...
               //href = _defaultHref;
               buttons = "<a data-handler='"+i+"' class='"+_class+"'>"+icon+""+label+"</a>" + buttons;
            }
        ...
        if (options['header']) {
            ...
            parts.push("<div class='modal-header'>"+closeButton+"<h5 class='modal-title'>"+options['header']+"</h5></div>");
        }
        ...
        div.off("keyup").on("keyup",function(e){
        	var key = e.which;
        	if(key == 27){
        		div.modal("hide");
        	}
